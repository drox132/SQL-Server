/*1.Utilizar Indice Clustered y Forzar El Indice A utilizarlo*/
SET STATISTICS IO ON
SET STATISTICS TIME ON
SET DATEFORMAT DMY;
--sp_help 'tblventas_detalle'
--DROP INDEX IX_TBLVENTAS_DETALLE_ID ON TBLVENTAS_DETALLE
--CREATE CLUSTERED  INDEX IX_TBLVENTAS_DETALLE_ID ON TBLVENTAS_DETALLE(ID,CANTIDAD,VALOR) 
...
FROM(
			...
			from tblVentas tv WITH(FORCESEEK)
				 INNER JOIN
			   tblVentas_Detalle tvd WITH(FORCESEEK) ON tv.ID = tvd.ID
